...
def sample_ddim(model, state_dict, shape, num_steps, schedule, device):
    ...
    for time, time_next in time_pairs:
        ...
        pred_noise = model(x_t, time_cond, state_dict, force_drop_goal=False)
...
def main(args):
    ...
    pca_path = os.path.join(PROJECT_ROOT, 'models', 'trajectory_pca.pkl')
    with open(pca_path, 'rb') as f:
        pca = pickle.load(f)
    print("Loaded normalization stats and fitted PCA model.")
    
    # --- Load Data ---
    val_files = glob(os.path.join(config['data']['featurized_v3_final_waypoint'], 'validation', '*.pt'))
    latent_stats_path = os.path.join(PROJECT_ROOT, 'models', 'latent_stats.pt')
    val_dataset = InMemoryDiffusionDataset(val_files[:100], stats_path=stats_path, pca_path=pca_path, latent_stats_path=latent_stats_path)
    ...
    for batch in tqdm(val_loader, desc="Evaluating on validation set"):
        ...
        gt_latent = batch['target_trajectory'] # Ground truth is now a latent vector        
        latent_shape = gt_latent.shape # (B, 16)


        # Generate K predictions for this scene
        predictions_k_latent = []
        for _ in range(eval_cfg['k_samples']):
            if args.sampler == 'ddim':
                pred_latent = sample_ddim(
                    model, state_dict, latent_shape, args.steps, schedule, device
                )
            elif args.sampler == 'ddpm':
                # DDPM uses the full schedule, so the `steps` argument is ignored.
                pred_latent = sample_ddpm(
                    model, state_dict, latent_shape, schedule, device
                )
            else:
                raise ValueError(f"Unknown sampler: {args.sampler}")
            predictions_k_latent.append(pred_latent)
            
        predictions_k_latent = torch.cat(predictions_k_latent, dim=0).cpu().numpy()
        
        # 1. Inverse transform from latent space to normalized trajectory space
        reconstructed_flat = pca.inverse_transform(predictions_k_latent) # (K, 160)
        reconstructed_normalized = torch.from_numpy(reconstructed_flat).view(-1, 80, 2).to(device)
        
        # 2. De-normalize from [-1, 1] back to meters
        predictions_k_meters = denormalize_trajectory(reconstructed_normalized, stats)
        
        # We also need to de-normalize the ground truth for comparison
        gt_flat = pca.inverse_transform(gt_latent.cpu().numpy())
        gt_normalized = torch.from_numpy(gt_flat).view(-1, 80, 2).to(device)
        gt_trajectory_meters = denormalize_trajectory(gt_normalized, stats)

        # Metrics are calculated on the final meter-space trajectories
        min_ade, min_fde = compute_multimodal_metrics(
            predictions_k_meters, gt_trajectory_meters.squeeze(0)
        )
